...
ROLLOUT_ARGS=(
   ...
   --prompt-data /root/slime/tests/spec/single.jsonl
   --input-key prompt
   --label-key label
   --apply-chat-template
   --rollout-shuffle
...
TMP_ARGS=(
   # --debug-rollout-only
   # --save-debug-rollout-data /root/slime/debug/1/data_{rollout_id}.pt
   --debug-train-only
   # --load-debug-rollout-data /root/slime/debug/1/data_{rollout_id}.pt
   --load-debug-rollout-data /root/slime/debug/1/data_union_{rollout_id}.pt
)
